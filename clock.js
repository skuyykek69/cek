...
document.body.style.setProperty("--percentage-of-day", `${getPercentageOfDay()}`);
...
function updateBgColor(percentage) {
  ...
  const dayColor = getComputedStyle(document.querySelector("body"))
    .getPropertyValue("--clr-d")
    .trim()
    .slice(1)
    .match(/.{2}/g)
    .map((hex) => parseInt(hex, 16));
  const nightColor = getComputedStyle(document.querySelector("body"))
